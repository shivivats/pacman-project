...
def createTeam(firstIndex, secondIndex, isRed,
               first = 'Group27OffensiveAgent', second = 'Group27DefensiveAgent'):
  """
...
# aka Lyza
class Group27OffensiveAgent(CaptureAgent):
  """
            ...
            if self.red:
                if gameState.getAgentState(self.index).getPosition()[0] > self.boundary[0][0]: # change this to work with blue as well
                    ...
                    if gameState.getAgentState(self.index).numCarrying > 0:
                        minDis, nearestDoor = self.getNearestTarget(gameState, myPos, self.behindBoundary)
                        minDis, action = self.getBestAction(gameState, nearestDoor, actions)
...
# aka Ozen
class Group27DefensiveAgent(CaptureAgent):
  """
  A Dummy agent to serve as an example of the necessary agent structure.
  You should look at baselineTeam.py for more details about how to
  create an agent as this is the bare minimum.
